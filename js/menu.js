...
const Menu = (() => {
  ...
  /* ---------- exam dates (fixed) ---------- */
  const PRELIMS_DATE = new Date(2027, 4, 23);   // 23 May 2027
  const MAINS_DATE = new Date(2027, 7, 20);     // 20 Aug 2027
  const PRELIMS_DAYS = [23];                    // highlighted in May 2027
  const MAINS_DAYS = [20, 21, 22, 28, 29];      // highlighted in Aug 2027
  ...
  function renderCountdowns() {
    ...
    if (p) p.textContent = daysLeft(PRELIMS_DATE);
    if (m) m.textContent = daysLeft(MAINS_DATE);
  }
  ...
  function buildCalendar(year, month, highlightDays) {
    ...
    for (let d = 1; d <= total; d++) {
      ...
      cells += `<div class="cal-cell${hot}">${d}</div>`;
    }
    return `
  ...
  function renderCalendars() {
    ...
    if (may) may.innerHTML = buildCalendar(2027, 4, PRELIMS_DAYS);   // May 2027
    if (aug) aug.innerHTML = buildCalendar(2027, 7, MAINS_DAYS);     // Aug 2027
  }
  ...
  function setTopics(arr) { localStorage.setItem(TOPICS_KEY, JSON.stringify(arr)); }
  ...
  function migrateLegacyNotes() {
    ...
    if (Array.isArray(legacy) && legacy.length) {
      const notes = legacy.map((n) => typeof n === "string"
        ...
        : n);
      localStorage.setItem(NOTE_KEY_PREFIX + gid, JSON.stringify(notes));
      localStorage.removeItem(LEGACY_NOTES_KEY); // remove old flat key (now migrated)
    }
  }
  ...
  function setNotes(topicId, arr) { localStorage.setItem(noteKey(topicId), JSON.stringify(arr)); }
