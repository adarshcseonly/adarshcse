...
const Tracker = (() => {
    ...
    grid.innerHTML = list.map((s, i) => {
      ...
      return `
        <div class="subject-card ${complete ? "is-complete" : ""}" data-id="${s.id}"
             tabindex="0" role="button" style="animation-delay:${i * 35}ms"
  ...
  function toggleChapter(subjectId, chapterId) {
    ...
    persist();
    renderAll();
    // refresh derived Revision/Mock modules if present
  ...
  return {
    init, renderAll, getSubject, getSubjectStats: (id) => {
    ...
    completedPrelimsChapters, prelimsChapterTitles,
  };
})();
